...
```{r mods, echo = FALSE, message = FALSE, cache = TRUE}
xml <- xmlParse("apple_health_export/export.xml")
df <- XML:::xmlAttrsToDataFrame(xml["//Record"])
hr <- df %>% filter(type == "HKQuantityTypeIdentifierHeartRate")
...
```{r plot1, echo = FALSE}
the_day <- plot_day(df, "2019-02-15", events = TRUE, hour_max = 10)
the_day_detail <- plot_day(df, "2019-02-15", events = TRUE, hour_min = 5, hour_max = 6, pt_size = 0.7)
