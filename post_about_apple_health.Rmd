...
First let's focus on items where where multiple measurements are taken over the
course of the day. A prime example is heart rate, which records the pulse at
a particular point in time. But there are a number of items
...
Each measurement for these items has a start time stamp and an end time stamp. In
R terms the time stamp is a `datetime` class. We have already spent a lot
of effort correcting these datetime stamps for movements across time zones and daylight savings.
Next, let's examine these measurements in more detail. We have already 
...
try4 <- movement %>% 
  filter(type %in% c("Active Energy", "Heart Rate", "Basal Energy", 
  ...
  summarise(Active_Energy = sum(type == "Active Energy"),
            Basal_Energy = sum(type == "Basal Energy"),
             Heart_Rate = sum(type == "Heart Rate"),
            Steps = sum(type == "Steps Watch"),
            Distance = sum(type == "Distance Watch"),
           ) %>% 
...
Spoiler alert: if you explicitly start a workout on the watch 
measurements are taken much more frequently. 
As a result the pattern of frequency of measurments
is dramatically different depending on whether or not a workout is in progress.
It doesn't makes sense to examine frequency of measurement
without identifying whether measurements happened during a workout.

It's tricky to identify which measurements were
taken during workouts. We have to combine the health_df table with the workouts table. For
each workout there is a start and an end time. Next I will join the `health_df` table
...
To do that I used the [fuzzyjoin package](https://github.com/dgrtwo/fuzzyjoin) by Dave Robinson.
It provides a variety of special joins that do not rely on an exact match.
In our case, we will use the `interval_left_join` function which joins tables based
...
impressed by the speed of `interval_left_join`. I thoght it would be impractical to run it
